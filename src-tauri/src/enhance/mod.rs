...
async fn collect_profile_items() -> Result<ProfileItems> {
    ...
    // FORK: prefer multi-merge result when active, else current profile mapping
...
// 吸收上游的 Result 错误传播（collect_profile_items 用 ?），保留 fork 第 4 元素 conflicts
pub async fn enhance() -> Result<(
    ...
    Ok((config, exists_keys_set, result_map, conflict_entries))
}
